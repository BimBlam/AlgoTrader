"""Tests for s1_orchestrator.process_manager."""
...
from s1_orchestrator.process_manager import ProcessManager
...
class TestLaunchWorker:
    def test_launch_creates_handle(self):
        ...
        with patch("s1_orchestrator.process_manager.subprocess.Popen", return_value=mock_proc):
    ...
    def test_launch_with_extra_env(self):
        ...
        with patch("s1_orchestrator.process_manager.subprocess.Popen", return_value=mock_proc) as popen:
...
class TestCollectFinished:
    def test_finished_workers_removed_from_registry(self):
        ...
        with patch("s1_orchestrator.process_manager.subprocess.Popen", return_value=proc):
