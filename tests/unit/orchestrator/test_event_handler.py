...
class TestModeChangedHandling:
    def test_mode_changed_calls_on_mode_changed_callback(self):
        ...
        with patch("algotrader.orchestrator.event_handler.get_session", return_value=session), \
             patch("algotrader.orchestrator.event_handler.get_config", return_value=mock_cfg), \
             patch("algotrader.orchestrator.event_handler.invalidate_cache"):
    ...
    def test_invalidate_cache_called_on_mode_change(self):
        ...
        with patch("algotrader.orchestrator.event_handler.get_session", return_value=session), \
             patch("algotrader.orchestrator.event_handler.get_config", return_value=mock_cfg), \
             patch("algotrader.orchestrator.event_handler.invalidate_cache") as mock_inv:
            handler = EventHandler(
