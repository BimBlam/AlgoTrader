"""Tests for algotrader.orchestrator.watchdog."""
...
class TestWatchdogStaleJobDetection:
    def test_stale_jobs_are_reset(self):
        ...
        with patch("algotrader.orchestrator.watchdog.get_session", return_value=mock_session), \
             patch("algotrader.orchestrator.watchdog.get_stale_running_jobs", return_value=[stale_job]), \
             patch("algotrader.orchestrator.watchdog.mark_job_failed") as mock_fail:
    ...
    def test_no_stale_jobs_does_nothing(self):
        ...
        with patch("algotrader.orchestrator.watchdog.get_session", return_value=mock_session), \
             patch("algotrader.orchestrator.watchdog.get_stale_running_jobs", return_value=[]), \
             patch("algotrader.orchestrator.watchdog.mark_job_failed") as mock_fail:
...
class TestWatchdogCriticalEventDetection:
    def test_critical_event_triggers_halt(self):
        ...
        with patch("algotrader.orchestrator.watchdog.get_session", return_value=session):
