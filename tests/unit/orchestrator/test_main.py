"""
Integration-level unit tests for algotrader.orchestrator.main.Orchestrator.
...
class TestOrchestratorStateTransitions:
    ...
    def test_force_halt_sets_halt_state(self):
        ...
        with patch("algotrader.orchestrator.main.get_session", return_value=mock_session):
...
class TestDispatchJobs:
    ...
    def test_dispatch_skipped_when_disabled(self):
        ...
        with patch("algotrader.orchestrator.main.get_config", return_value=mock_cfg):
            with patch.object(orch, "_launch_single_job", side_effect=lambda *a, **kw: dispatched.append(a)):
                orch._dispatch_jobs(["INGEST_EOD"])
        assert dispatched == []
...
class TestHandleWorkerExit:
    ...
    def test_exit_code_0_calls_mark_completed(self):
        ...
        with patch("algotrader.orchestrator.main.get_session", return_value=mock_session):
            with patch("algotrader.orchestrator.main.get_job_by_run_id", return_value=job):
                with patch("algotrader.orchestrator.main.mark_job_completed") as mock_complete:
                    with patch("algotrader.orchestrator.main.mark_job_failed") as mock_fail:
    ...
    def test_ingest_failure_halts_when_configured(self):
        ...
        with patch("algotrader.orchestrator.main.get_session", return_value=mock_session):
            with patch("algotrader.orchestrator.main.get_job_by_run_id", return_value=job):
                with patch("algotrader.orchestrator.main.mark_job_failed"):
                    with patch("algotrader.orchestrator.main.get_config", return_value=mock_cfg):
...
class TestEnterApprovalState:
    def test_transitions_to_pending_approval(self):
        ...
        with patch("algotrader.orchestrator.main.get_session", return_value=mock_session):
            with patch("algotrader.orchestrator.main.ApprovalManager") as mock_am_cls:
                mock_am = MagicMock()
