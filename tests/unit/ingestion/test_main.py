...
from algotrader.ingestion import main
...
def patched_env(mock_cfg):
    ...
    with (
        patch("algotrader.ingestion.main.get_config", return_value=mock_cfg),
        patch("algotrader.ingestion.main.init_db"),
        patch("algotrader.ingestion.main.get_session"),
        patch("algotrader.ingestion.main.SystemEvent"),
    ):
        yield
...
class TestRunSuccess:
    def test_data_ready_emitted_on_full_success(self, patched_env, mock_cfg, today):
        with (
            patch("algotrader.ingestion.main.download_and_persist_ohlcv", return_value=pd.DataFrame()),
            patch("algotrader.ingestion.main.validate_ohlcv", return_value=[]),
            patch("algotrader.ingestion.main.compute_and_write_returns"),
            patch("algotrader.ingestion.main.scrape_news"),
            patch("algotrader.ingestion.main.scrape_social"),
            patch("algotrader.ingestion.main._emit_event") as mock_emit,
    ...
    def test_data_stale_emitted_when_one_ticker_fails(self, patched_env, today):
      ...
      with (
          patch("algotrader.ingestion.main.get_config", return_value=cfg_5),
          patch("algotrader.ingestion.main.init_db"),
          patch("algotrader.ingestion.main.get_session"),
          patch("algotrader.ingestion.main.SystemEvent"),
          patch("algotrader.ingestion.main.download_and_persist_ohlcv", side_effect=fail_first_ticker),
          patch("algotrader.ingestion.main.validate_ohlcv", return_value=[]),
          patch("algotrader.ingestion.main.compute_and_write_returns"),
          patch("algotrader.ingestion.main.scrape_news"),
          patch("algotrader.ingestion.main.scrape_social"),
...
class TestAbortThreshold:
    def test_data_error_and_raises_when_over_20pct(self, patched_env, mock_cfg, today):
        ...
        with (
            patch("algotrader.ingestion.main.download_and_persist_ohlcv",
                  side_effect=RuntimeError("all fail")),
            patch("algotrader.ingestion.main._emit_event"),
            patch("algotrader.ingestion.main.datetime") as mock_dt,
        ):
            mock_dt.datetime.now.return_value.date.return_value = today
            mock_dt.timezone = datetime.timezone
    ...
    def test_does_not_raise_when_one_of_five_fails(self, patched_env, today):
        ...
        with (
            ...
            patch("algotrader.ingestion.main.get_session"),
            patch("algotrader.ingestion.main.SystemEvent"),
            patch("algotrader.ingestion.main.download_and_persist_ohlcv", side_effect=partial_fail),
            patch("algotrader.ingestion.main.validate_ohlcv", return_value=[]),
            patch("algotrader.ingestion.main.compute_and_write_returns"),
            patch("algotrader.ingestion.main.scrape_news"),
            patch("algotrader.ingestion.main.scrape_social"),
