...
from s6_execution.reconcile import _last_fill_price, _reconcile
...
class TestReconcile:
    @patch("s6_execution.reconcile.expire_stale_signals", return_value=0)
    @patch("s6_execution.reconcile.write_event")
    @patch("s6_execution.reconcile.close_position")
    @patch("s6_execution.reconcile.get_session")
    ...
    @patch("s6_execution.reconcile.expire_stale_signals", return_value=3)
    @patch("s6_execution.reconcile.write_event")
    @patch("s6_execution.reconcile.close_position")
    @patch("s6_execution.reconcile.get_session")
